...
const NAV_LINKS = MAIN_PAGES.filter((p) => !!p.label).map((p) => ({
  id: p.name,
  name: p.label!,
  to: { name: p.name },
  exact: !!p.exact,
...
<template>
  <header
    class="sticky top-0 py-4 z-50 w-full flex items-center bg-black text-gray-100 px-5.5 lg:px-14"
  >
    ...
    <div
      v-show="open"
      class="absolute rounded-b-lg top-18 left-0 w-full h-[70vh] bg-black sm:hidden"
    >
